...
/*   Updated: 2020/03/17 17:04:59 by lniki            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void    print_with_precf_xx(t_pr *mod, t_spec_o *spec)
{
...
void    if_no_minus_xx(t_pr *mod, t_spec_o *spec)
{
     if(mod->minus == 0)
    {
        ...
        print_with_precf_xx(mod, spec);
    }
}

void    if_minus_xx(t_pr *mod, t_spec_o *spec)
{
    if(mod->minus == 1)
    {
...
void	print_zero_hash_precf_xx(t_pr *mod, t_spec_o *spec)
{
...
void    tipe_xx(t_pr *mod, t_spec_o *spec)
{
    if(mod->leng == 1)
...
void   print_xx(t_pr *mod)
{
    ...
    tipe_xx(mod, &spec);
    ft_xx(&spec);
    print_zero_hash_precf_xx(mod, &spec);
    if_minus_xx(mod, &spec);
    if_no_minus_xx(mod, &spec);
    free(spec.s);
}